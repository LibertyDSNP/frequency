...
fn require_valid_schema_size_errors() {
	new_test_ext().execute_with(|| {
		...
		let test_cases: [TestCase<(Error<Test>, u8)>; 2] = [
			...
			TestCase {
				input: r#"{"id": "long", "title": "I am a very very very very very long schema", "properties": "just way too long to live a long life", "description": "Just a never ending stream of bytes that goes on for a minute too long"}"#,
